# 最后的书店 / The Last Bookstore

> 腾讯云 AI 黑客松 40 赛道 · 叙事剧情游戏 · AI CAN DO IT
> 大断联后的第 7 年，云端知识沉默，纸质书重新成为人类能握在手里的记忆。
> 你能不能在第 7 天之前，点燃这个小镇最后一点希望？

## 🎮 立即体验

**👉 <https://the-last-bookstore.vercel.app/?demo=1>**

（Vercel 正式版，包含服务器端 MiniMax 实时对话；浏览器直接打开，无需配置 API Key。）

静态备份：<https://swording-k.github.io/the-last-bookstore/>（游戏可玩；AI 请求通过 Vercel Serverless 代理转发，无需配置 API Key。）

> 戴上耳机、关灯、把窗口放到最大 — BGM + 暖色场景 + 打字机叙事的沉浸感比录屏好十倍。
> 完整一周目约 25 分钟。

### 本地运行

```bash
git clone https://github.com/swording-k/the-last-bookstore.git
cd the-last-bookstore
npx vercel dev --listen 8080
# 打开 http://localhost:8080/；MiniMax 密钥放在被 gitignore 的 .env.local
```

## 关于

7 天。24 本书。6 个灵魂。1 间还在营业的书店。

你扮演橡木镇唯一还在营业的书店老板。每天门铃会响，会有带着故事的人走进来 — 疲惫的母亲、孤独的老农、自卑的小孩、超负荷的医生、迷惘的女诗人、神秘的旅人。你要做的不只是卖书，而是**听懂他们**，**推荐对的那一本**。

每一次对话、每一次选择都会影响 **希望值（hope）** 和 **声誉（rep）**。7 天后，4 种结局之一等着你。

## 怎么玩

1. 点 **推开那扇门** 开始
2. 每天开场看一段**叙事过场**（大断联背景介绍 + 当日目标）
3. 门铃响 → **接待** → 看 NPC 故事 → **选回应**（三个分支按钮）
4. 想聊点不一样的？点 **💬 和 TA 自由交谈**，AI 真 NPC 实时对话
5. 选完后 **推荐一本书** → 在 24 本书里选 → 看匹配度（完美 / 良好 / 中性）
6. 一本书完成 → 继续接待下一个
7. 一天结束 → **看店主日记**（AI 生成）→ 推进到下一天
8. 第 7 天结束 → 触发结局
9. 随时点右上角 **📚** 打开 **我的书店** → 查看当前推荐记录、待读清单和一个月后的回信
10. 随时点右上角 **📖** 打开 **自由书库** → 不等 NPC，直接查看书、AI 导读，或进入《福尔摩斯探案集》的书中剧场

## 4 种结局

| 结局 | 条件 | 感觉 |
| --- | --- | --- |
| **平凡日子** | 7 天后希望值 40-60 | 你只是个守店的 |
| **守望者** | 7 天后希望值 60-80 | 你让几个人不再孤单 |
| **点燃希望** ✦ TRUE | 第 6+ 天 + hope≥80 + 见过旅人 | 整个小镇都变了 |
| **最后的读者** 🌙 HIDDEN | 第 7 天 + hope≥60 + 见过旅人 + 5+ 完美 | 旅人留下的旧书，笔迹是祖父的 |

## AI 用了哪里？

- **6 张 NPC 写实立绘**（matrix AI 出图，写实肖像风格，3 轮 prompt 迭代，PIL 去白底处理）
- **3 张书中剧场角色立绘**（福尔摩斯、华生、雷斯垂德 — 19世纪侦探素描风格）
- **24 张书封面**（matrix AI）
- **4 段 BGM / 4 个 SFX**（matrix AI 音乐生成）
- **NPC 自由对话**（MiniMax-M2.5-highspeed 流式，服务器代理保护密钥）
- **书籍 AI 导读**（同上，问作者问题、聊书背后故事）
- **每日店主日记**（模板 + LLM 润色）
- **结果世界风注释**（完美/良好/中性匹配各一套氛围文案）
- **一个月后回信**（推荐记录生成后续回信，LLM 不可用时走本地 fallback）

### 立绘动态效果

NPC 立绘在对话和书中剧场中支持：
- **呼吸动画**：细微的缩放脉冲，让立绘"活"起来
- **淡入效果**：立绘切换时的平滑过渡
- **双层阴影**：增强立体感和沉浸式体验

## 新版本亮点：我的书店

结局不再只是结束。玩家可以打开 **我的书店**，看到自己七天里推荐过哪些书、把哪些书加入了“我想读”的待读清单，并复制书名去微信读书、图书馆或线下书店搜索。

这让《最后的书店》从剧情游戏变成真实阅读入口：游戏不是终点，它只是把玩家带回真实阅读的入口。

现场演示时不必玩到结局。打开 `/?demo=1` 后，顶部工具条可以一键跳到结局或打开「我的书店」；正常游戏中也可以随时点右上角 📚 查看当前进度。

## 新版本亮点：自由书库与书中剧场 v4.0

玩家不必等到 NPC 推荐环节，也可以随时点右上角 **📖** 打开自由书库。自由浏览模式下，书籍详情页不会出现"推荐这本书"的任务按钮，只保留阅读相关互动：书籍简介、AI 导读，以及带有 `bookWorld` 的沉浸式入口。

当前自由书库会把带有沉浸式剧场的书置顶，并显示 **沉浸式剧场推荐**。第一批演示书有两本：

- 《福尔摩斯探案集》：进入贝克街 221B，与夏洛克·福尔摩斯、华生医生、雷斯垂德探长进行卡片式或自由提问式对话。
- 《小王子》：进入 B612 星球，与小王子、狐狸、玫瑰聊孤独、爱、责任和"真正重要的东西"。

没有 API Key 时也有角色化 fallback，有 Key 时会走真实 LLM。

### 书中剧场 v4.0：专业视觉小说（VN）体验

书中剧场已重构为**左立绘 + 右对话框**的经典视觉小说布局：

- **左侧角色立绘**：写实风格 NPC 肖像，支持呼吸动画和淡入效果
- **右侧对话面板**：名字标签 + 逐字打字机效果 + 探索热点
- **场景背景**：每本书有专属场景图，沉浸感更强
- **多角色切换**：同一剧场中可与多个角色对话，角色立绘平滑过渡
- **自由对话模式**：不止预设剧情，还可以和书中角色自由聊天

技术实现：`booktheater.js` 状态机驱动，CSS `portrait-breathe` 呼吸动画 + `portrait-enter` 淡入动画，双层 `drop-shadow` 增强立体感。

详细：[docs/AI-CREATION.md](docs/AI-CREATION.md)

## 技术

- 纯 HTML / CSS / 原生 JS，**零构建**
- 全部源码 < 100KB（不含资源）
- localStorage 3 槽存档
- **离线可玩**（AI 部分有本地 fallback，断网也不卡）
- 6 NPC + 24 书 + 4 结局 = 完整可重玩

工程文档：[docs/DEV.md](docs/DEV.md)
原始 GDD：[docs/GDD.md](docs/GDD.md)

## 📺 作品演示

**评委专用演示页**（HTML 幻灯片，键盘翻页，含游戏截图）：  
👉 <https://swording-k.github.io/the-last-bookstore/presentation.html>

## 部署

本项目的 **GitHub Pages 是纯静态托管**，无法直接运行后端代码。AI 功能通过 **Vercel Serverless 函数代理**实现：

| 平台 | 用途 | 链接 |
| --- | --- | --- |
| **Vercel** | 静态托管 + `api/chat.js` Serverless 函数（持有 MiniMax API Key） | <https://the-last-bookstore.vercel.app/> |
| **GitHub Pages** | 静态托管（前端页面）| <https://swording-k.github.io/the-last-bookstore/> |

### AI 请求路由逻辑

`llm.js` 在运行时会自动检测当前域名：
- 如果在 **`github.io`** 域上运行 → AI 请求跨域发到 `https://the-last-bookstore.vercel.app/api/chat`
- 如果在 **Vercel** 或 **本地** 运行 → 使用相对路径 `/api/chat`

API Key 始终安全存储在 **Vercel 环境变量**中，不会暴露给浏览器。

技术细节：
- `api/chat.js` 已配置 **CORS 白名单**（`swording-k.github.io`、Vercel 自身域名、本地开发地址）
- 支持 **OPTIONS 预检请求**（浏览器跨域必需）
- 两个网址打开后 **AI 功能均可正常使用**

Vercel 部署配置已包含在 `vercel.json` 中，一键导入即可。

## 演示真实 LLM

本仓库不提交 API Key。没有 Key 时，NPC 对话、书籍导读和一个月后回信都会走本地 fallback，断网也能展示。

需要现场演示真实流式 LLM 时，可以在浏览器控制台临时设置：

```js
localStorage.setItem('tlb_llm_api_key', '你的临时演示 Key')
```

也可以复制 `demo-key.local.example.js` 为 `demo-key.local.js`，把临时 Key 填进去。本文件已被 `.gitignore` 忽略，只用于本机演示。

更稳妥的线上做法是把 Key 放在服务端或部署平台环境变量里，再由服务端代理请求。

## 控制台快捷键

```js
Engine.state                          // 查看当前状态
Engine.state.hope = 99               // 强行改希望值（debug 用）
Engine.state.metTraveler = true      // 见过旅人
Game._showNight()                    // 强制触发夜间面板
AudioManager.toggleMute()             // 切换静音
Renderer.skipTyping()                 // 跳过打字机
Renderer.showScreen('ending-screen') // 直接跳到结局页
```

## 致谢

- 腾讯云 AI 黑客松
- 矩阵 AI (matrix MCP) — 出图 / 出音乐
- MiniMax — MiniMax-M2.5-highspeed 流式对话
- Codex — 联调优化（叙事过场 / AI 接入 / 结局平衡）
- 那个让我们坚持把《最后》做完的下午

---

_— 2026.06.22 —_
